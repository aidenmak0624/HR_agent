...
class RAGSearchTool:
    ...
    def _assess_quality(self, distances: List[float], documents: List[str]) -> Dict[str, Any]:
        ...
        overall_confidence = (
            distance_score * 0.35 +      # 35% weight: average quality
            coverage_score * 0.25 +      # 25% weight: how many are good
            best_match_score * 0.25 +    # 25% weight: best result quality
            consistency_score * 0.15     # 15% weight: consistency
        )
    ...
    def _get_recommendation(self, metrics: Dict[str, Any], is_sufficient: bool) -> str:
        ...
        if is_sufficient:
            ...
            if metrics['num_relevant'] < self.MIN_CHUNKS:
                issues.append(f"insufficient coverage ({metrics['num_relevant']}/{self.MIN_CHUNKS})")

...
class ComparatorTool:
    ...
    def _retrieve_info(self, item: str, topic: str) -> Dict[str, Any]:
        ...
        results = self.rag.retrieve(
            query=item,
            topic=topic,
            n_results=8  # Get multiple perspectives
        )
    ...
    def _has_sufficient_info(self, info_a: dict, info_b: dict) -> bool:
        ...
        return (
            len(info_a.get('documents', [])) >= 2 and
            len(info_b.get('documents', [])) >= 2
        )
    
    def _compare_with_llm(self, item_a: str, info_a: dict,
                          item_b: str, info_b: dict,
...
**YOUR COMPARISON:**"""
    ...
    def _fallback_comparison(self, item_a: str, info_a: dict,
                            item_b: str, info_b: dict) -> Dict[str, Any]:
        """

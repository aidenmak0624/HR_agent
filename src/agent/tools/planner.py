...
class EducationalPlannerTool:
    ...
    def run(
        self, content_type: str, topic: str, level: str = "high_school", details: dict = None
    ) -> Dict[str, Any]:
    ...
    def _gather_topic_info(self, topic: str) -> dict:
        ...
        try:
            results = self.rag.retrieve(
                query=f"Educational content about {topic}", topic=topic, n_results=5
            )
    ...
    def _generate_lesson_plan(
        self, topic: str, level: str, topic_info: dict, details: dict
    ) -> Dict[str, Any]:
...
**LESSON PLAN:**"""
        ...
        try:
            messages = [
                SystemMessage(
                    content="You are an expert HR trainer, specializing in employee training and development. Generate the lesson plan in clear markdown format."
                ),
...
Return ONLY JSON array of question objects, where each object has 'question', 'options' (list of strings), 'answer' (string matching an option), and 'explanation'."""
        ...
        try:
            messages = [
                SystemMessage(content="You are a quiz creator who returns valid, clean JSON."),
                HumanMessage(content=prompt),
            ]
            response = self.llm.invoke(messages)
            ...
            try:
                ...
                return {"content": quiz_data, "format": "json", "sources": topic_info["sources"]}
            except json.JSONDecodeError:
                logger.warning(
                    "Quiz generation failed to return clean JSON. Returning as Markdown."
                )
                return {
                    "content": response.content,
                    "format": "markdown",
...
Include: Key Concepts, Definitions, Study Questions, Resources (links are optional if not in context)"""
        ...
        try:
            messages = [
                SystemMessage(
                    content="You are an expert educator who generates helpful study guides in clear markdown."
                ),
                HumanMessage(content=prompt),
            ]
            response = self.llm.invoke(messages)
...
if __name__ == "__main__":
    ...
    result = planner.run(content_type="lesson_plan", topic="benefits", level="high_school")

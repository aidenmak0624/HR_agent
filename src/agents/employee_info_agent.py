...
class EmployeeInfoAgent(BaseAgent):
    ...
    def get_tools(self) -> Dict[str, Any]:
            ...
            try:
                ...
                if search_type == "department" or search_type == "auto":
                    ...
                    if org_nodes:
                        return {
                            "org_nodes": [
                                n.dict() if hasattr(n, "dict") else str(n) for n in org_nodes
                            ],
                            "count": len(org_nodes),
                    ...
                    if managers:
                        ...
                        reports = [
                            n for n in org_nodes if getattr(n, "manager_id", None) == manager.id
                        ]
                        return {
                            "manager": manager.dict(),
                            "direct_reports": [
                                r.dict() if hasattr(r, "dict") else str(r) for r in reports
                            ],
                            "count": len(reports),
        ...
        def profile_synthesizer(employee_data: Dict[str, Any]) -> Dict[str, str]:
            ...
            try:
                logger.info(
                    f"PROFILE_SYNTHESIZER: Generating summary for {employee_data.get('first_name', 'Unknown')}"
                )

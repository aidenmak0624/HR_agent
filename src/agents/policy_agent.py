...
class PolicyAgent(BaseAgent):
    ...
    def get_tools(self) -> Dict[str, Any]:
        ...
        def rag_policy_search(
            ...
            try:
                ...
                if employee_id and self.hris_connector:
                    try:
                        ...
                        if emp:
                            ...
                            logger.info(
                                f"RAG_SEARCH: Enriched with employee context for {employee_id}"
                            )
                ...
                results = self.rag_pipeline.search(
                    query=enriched_query, collection=collection_name, top_k=top_k, min_score=0.3
                )
        ...
        def compliance_check(scenario: str, policy_query: Optional[str] = None) -> Dict[str, Any]:
            ...
            try:
                ...
                results = self.rag_pipeline.search(
                    query=search_query, collection="policies", top_k=5, min_score=0.3
                )
                ...
                try:
                    ...
                    if jurisdiction_check:
                        jurisdiction_results = (
                            jurisdiction_check
                            if isinstance(jurisdiction_check, list)
                            else [jurisdiction_check]
                        )
    ...
    def _plan_node(self, state: BaseAgentState) -> BaseAgentState:
        ...
        if any(
            word in query for word in ["compliant", "compliance", "allowed", "permitted", "can i"]
        ):
            ...
            word in query for word in ["find", "search", "look up", "what is", "tell me about"]
        ):
        ...
        state.setdefault("reasoning_trace", []).append(
            f"Created policy plan with {len(plan)} steps"
        )

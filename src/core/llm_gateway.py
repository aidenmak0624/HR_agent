...
class LLMGateway:
    ...
    DEFAULT_MODELS: Dict[TaskType, ModelConfig] = {
        TaskType.CLASSIFICATION: ModelConfig(
            model_name="gpt-4o-mini", temperature=0.1, max_tokens=256, timeout_seconds=10
        ),
        TaskType.SYNTHESIS: ModelConfig(
            model_name="gpt-4o-mini", temperature=0.3, max_tokens=2048, timeout_seconds=30
        ),
        ...
        TaskType.COMPLIANCE: ModelConfig(
            model_name="gpt-4o-mini", temperature=0.0, max_tokens=1024, timeout_seconds=20
        ),
        TaskType.REFLECTION: ModelConfig(
            model_name="gpt-4o-mini", temperature=0.1, max_tokens=512, timeout_seconds=15
        ),
    }
    ...
    def send_prompt(self, task_type: TaskType, prompt: str, **kwargs: Any) -> LLMResponse:
        """
        ...
        if not self._is_circuit_available(model_config.model_name):
            logger.warning(f"Circuit breaker OPEN for model {model_config.model_name}")
            raise RuntimeError(
        ...
        for attempt in range(self.RETRY_ATTEMPTS):
            try:
                ...
                response_text = self._llm_call_handler(
                    model_config=model_config, prompt=prompt, **kwargs
                )
    ...
    def _default_llm_call(self, model_config: ModelConfig, prompt: str, **kwargs: Any) -> str:
        """
    ...
    def _save_to_cache(self, cache_key: str, response: LLMResponse) -> None:
        ...
        try:
            ...
            self.cache.setex(cache_key, 86400, json.dumps(cache_data))  # 24 hour TTL
        except Exception as e:
    ...
    def _record_success(
        self, model_name: str, latency_ms: float, tokens_in: int = 0, tokens_out: int = 0
    ) -> None:
    ...
    def _record_failure(self, model_name: str) -> None:
        ...
        if metrics.consecutive_failures >= self.CIRCUIT_BREAKER_THRESHOLD:
            ...
            self.circuit_breaker_reset_times[model_name] = datetime.now() + timedelta(
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for model_name, metrics in self.metrics.items():
            success_rate = (
                (metrics.success_count / metrics.call_count) if metrics.call_count > 0 else 0.0
            )
            avg_latency = (

...
class HallucinationDetector:
    ...
    def __init__(self):
        ...
        self.hedging_pattern = re.compile("|".join(self.HEDGING_PHRASES), re.IGNORECASE)
        self.unsupported_pattern = re.compile("|".join(self.UNSUPPORTED_INDICATORS), re.IGNORECASE)
    ...
    def find_issues(self, text: str) -> List[str]:
        ...
        if len(hedging_matches) > 3:
            issues.append(f"excessive_hedging ({len(hedging_matches)} phrases)")
...
class QualityAssessor:
    ...
    def assess(
        ...
        confidence = self._assess_confidence(response, tool_results=tool_results)
    ...
    def _assess_relevance(self, query: str, response: str) -> float:
        ...
        if any(
            neg in response.lower() for neg in ["i cannot", "i dont", "unable to", "not available"]
        ):
            negation_penalty = 0.2

        relevance = ((keyword_relevance * 0.6) + (response_length_score * 0.4)) - negation_penalty
    ...
    def _assess_confidence(
        self, response: str, tool_results: Optional[Dict[str, Any]] = None
    ) -> float:
        """
    ...
    def _assess_source_quality(
        ...
        if sources:
            ...
            if rag_sources:
                ...
                avg_relevance = sum(s.get("relevance_score", 0.5) for s in rag_sources) / len(
                    rag_sources
                )
    ...
    def suggest_fallback(self, quality_score: QualityScore) -> Optional[str]:
        """

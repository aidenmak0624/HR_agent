...
def test_simple_rag():
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE}/api/chat",
            json={"query": query, "topic": "benefits", "difficulty": "detailed"},
...
def test_agent_mode():
    ...
    try:
        ...
        response = requests.post(
            ...
            json={"query": query, "topic": "company_policies", "difficulty": "detailed"},
            headers={"Content-Type": "application/json"},
        ...
        if response.status_code == 200:
            ...
            print(f"\n{Fore.WHITE}Sources: {Style.RESET_ALL}{', '.join(data.get('sources', []))}")
            print(
                f"{Fore.WHITE}Tools used: {Style.RESET_ALL}{' → '.join(data.get('tools_used', []))}"
            )
            print(f"{Fore.WHITE}Confidence: {Style.RESET_ALL}{data.get('confidence', 0):.2f}")
...
def test_error_handling():
    ...
    try:
        response = requests.post(f"{API_BASE}/api/chat", json={"topic": "benefits"}, timeout=5)
    ...
    try:
        response = requests.post(
            f"{API_BASE}/api/agent/chat",
            json={"query": "test", "topic": "benefits", "difficulty": "invalid"},
            timeout=5,
        )
        if response.status_code == 400:

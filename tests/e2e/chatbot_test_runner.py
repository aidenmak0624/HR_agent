...
class ChatbotTestRunner:
    ...
    def evaluate_test(self, category: str, query: str, response: Dict, 
                     expected_agents: List[str], expected_confidence: float) -> Dict:
        ...
        if confidence >= expected_confidence:
            ...
            pass_conditions.append(False)
            test_result["reason"] += f"Confidence {confidence:.2f} below threshold {expected_confidence:.2f}. "
    ...
    def run_all_tests(self):
        ...
        for category_name, category_config in TEST_CATEGORIES.items():
            ...
            for query in category_config["queries"]:
                ...
                confidence_str = f"{test_result['confidence']:.2f}" if test_result['confidence'] is not None else "N/A"
                agent_str = test_result['agent_type'] if test_result['agent_type'] else "N/A"
                
                print(f"  Agent: {agent_str} | Confidence: {confidence_str} | Time: {response_time:.0f}ms")
    ...
    def print_summary_table(self):
        ...
        print("="*120)
        print(f"{'#':<4} {'Category':<20} {'Query':<35} {'Status':<6} {'Agent':<18} {'Conf':<6} {'Time(ms)':<8}")
        print("-"*120)
        ...
        for i, result in enumerate(self.results, 1):
            ...
            print(f"{i:<4} {result['category']:<20} {query_short} {status:<6} {agent:<18} {confidence:<6} {time_ms:<8}")
    ...
    def save_results(self):
        ...
        output = {
            "test_run_info": {
                ...
                "pass_rate": sum(1 for r in self.results if r['pass']) / len(self.results) * 100 if self.results else 0
            },
